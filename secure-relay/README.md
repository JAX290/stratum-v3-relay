...
设置确认无误后点击“安装后台服务”，在 Windows 提示时允许管理员权限。程序会：

1. 把服务程序和一份使用 Windows 机器密钥加密的配置放到 `C:\ProgramData\MulinSenRelay`；
2. 创建“木林森中转后台服务”；
3. 设置为开机自动运行；
4. 设置异常退出后的自动重启。

以后修改配置，先保存，再点击“更新后台服务”。更新时服务会短暂停止并重新启动，因此应安排在方便矿机重连的时间。后台服务运行时，打开桌面程序可以看到服务状态和最近的连接、流量及线路健康数据。桌面程序中的“开机自动启动”只控制登录 Windows 后是否自动打开管理界面；后台转发本身由 Windows 服务负责。
