...
项目使用 `CN=木林森中转` 的内部代码签名证书。签名私钥为不可导出，只保存在发布电脑的 Windows 证书库；`client/signing/木林森中转-内部代码签名证书.cer` 是不含私钥的公开证书，可以复制到矿场电脑。

新矿场电脑首次使用时，以管理员身份运行 `client/signing/安装木林森内部签名证书.ps1`。安装脚本会先核对固定证书指纹，再把公开证书加入本机的“受信任的根证书颁发机构”和“受信任的发布者”。以后只接受发布者为“木林森中转”且数字签名有效的 EXE。若电脑不再使用本程序，可以运行同目录的移除脚本撤销信任。
...
当前源码版本 `2.2.2` 完成中转核心、状态统计、历史存储和窗口源码分离，使用步骤、配置位置与主备连接行为保持兼容。本阶段尚未提供无需登录的 Windows 服务；验收与发布状态见[路线计划](../docs/improvement-roadmap.md)。开发与测试入口见[客户端源码结构](client/ARCHITECTURE.md)。
