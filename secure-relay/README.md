...
2.0 增加了主 VPS 和两个备用 VPS。2.1.5 在主、备用 VPS 设置旁增加了真实连接测试，并支持自定义自动探测间隔。测试会完整验证 TCP、TLS证书和共享密钥，并显示成功、失败原因、响应时间和检测时间。点击“备用 VPS 设置”，每个标签页都可以分别填写启用状态、VPS 地址、TLS 端口、证书名称、证书 SHA-256 和独立共享密钥。新矿机连接会先尝试主 VPS；主 VPS 无法连接、证书不符、密钥无效或没有对应路线时，依次尝试备用 VPS。主 VPS 恢复后，之后建立的新连接会重新优先使用主 VPS，已经稳定运行的连接不会被强制切断。
