...
估算算力使用已接受 Share 的难度和观察时间计算，口径为 `难度合计 × 2^32 ÷ 秒数`。它会受 Share 随机性影响，短时间数据可能明显波动，也不等同于矿机芯片直接上报的瞬时算力。程序必须持续运行并收到标准 Stratum `mining.set_difficulty`、`mining.submit` 和响应消息才能计算；当前版本重启程序后重新开始累计。
