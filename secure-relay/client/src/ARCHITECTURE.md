...
WIN-P01 已开始，已实现恢复策略、持久化互斥和独立服务／工作进程组件，尚未完成安装迁移和服务实机验收；边界与验证记录见 [服务阶段说明](SERVICE-DESIGN.md)。
...
核心每种构建 28 项检查覆盖配置兼容、端口映射、Share 接受和拒绝、分包解析、重复响应、历史窗口、快照隔离、在线矿机删除保护、端口冲突后的监听清理、重复停止和本地监听健康检查。测试不写入生产配置或历史，不启动生产界面，也不连接实际 VPS。额外运行恢复及服务组件测试，总计 122 项检查。
