...
if($LASTEXITCODE-ne 0){throw 'Signed update tests failed to compile.'}
& $signedUpdateTestExe
if($LASTEXITCODE-ne 0){throw 'Signed update tests failed.'}

$remoteControlTestExe=Join-Path $testDirectory 'remote-control-tests.exe'
& $compiler /nologo /target:exe /out:$remoteControlTestExe /reference:$coreLibrary (Join-Path $PSScriptRoot 'test_remote_control.cs')
