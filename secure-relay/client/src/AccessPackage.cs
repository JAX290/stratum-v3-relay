...
public static class AccessPackageCodec
{
    ...
    private static byte[] Decode(string value,int size){byte[] data;try{data=Convert.FromBase64String(value??"");}catch{throw new InvalidDataException("接入文件编码不正确。");}if(data.Length!=size)throw new InvalidDataException("接入文件字段长度不正确。");return data;}
    private static byte[] Decode(string value,int min,int max){byte[] data;try{data=Convert.FromBase64String(value??"");}catch{throw new InvalidDataException("接入文件编码不正确。");}if(data.Length<min||data.Length>max)throw new InvalidDataException("接入文件内容长度不正确。");return data;}
    private static byte[] Pbkdf2Sha256(byte[] password,byte[] salt,int iterations,int length){byte[] output=new byte[length];using(HMACSHA256 hmac=new HMACSHA256(password)){int offset=0,block=1;while(offset<length){byte[] counter={ (byte)(block>>24),(byte)(block>>16),(byte)(block>>8),(byte)block};byte[] u=hmac.ComputeHash(Join(salt,counter));byte[] value=(byte[])u.Clone();for(int round=1;round<iterations;round++){u=hmac.ComputeHash(u);for(int i=0;i<value.Length;i++)value[i]^=u[i];}int count=Math.Min(value.Length,length-offset);Buffer.BlockCopy(value,0,output,offset,count);offset+=count;block++;}}return output;}
    private static byte[] Keystream(byte[] key,byte[] nonce,int length){byte[] output=new byte[length];using(HMACSHA256 hmac=new HMACSHA256(key)){int offset=0,counter=1;while(offset<length){byte[] block=hmac.ComputeHash(Join(nonce,new[]{(byte)(counter>>24),(byte)(counter>>16),(byte)(counter>>8),(byte)counter}));int count=Math.Min(block.Length,length-offset);Buffer.BlockCopy(block,0,output,offset,count);offset+=count;counter++;}}return output;}
    private static byte[] Xor(byte[] left,byte[] right){byte[] output=new byte[left.Length];for(int i=0;i<left.Length;i++)output[i]=(byte)(left[i]^right[i]);return output;}
    private static byte[] Slice(byte[] value,int offset,int count){byte[] output=new byte[count];Buffer.BlockCopy(value,offset,output,0,count);return output;}
    private static byte[] Join(params byte[][] values){int size=0;foreach(byte[] value in values)size+=value.Length;byte[] output=new byte[size];int offset=0;foreach(byte[] value in values){Buffer.BlockCopy(value,0,output,offset,value.Length);offset+=value.Length;}return output;}
    private static bool FixedEquals(byte[] left,byte[] right){if(left.Length!=right.Length)return false;int difference=0;for(int i=0;i<left.Length;i++)difference|=left[i]^right[i];return difference==0;}
}
