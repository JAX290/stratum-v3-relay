...
4. 两台都保存后，只在当前配置正确的 VPS 上点击一次“立即同步本机全部线路”，等待页面明确显示同步成功；
5. 此后从任意一台修改、自动切换或恢复线路，页面都会立即等待对端确认并反馈结果；对端暂时离线时任务会保留并自动重试。

同步密钥只用于两台 VPS 之间验证身份，不要提交到 GitHub。设置页显示最近同步结果、反馈时间、待重试任务和已确认端口；同步结果无论成功或失败也会写入日志并在总览首页显示。
