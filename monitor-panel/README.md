...
4. 选择一台在线矿机，点击“开始单机试切”。系统只会让这台矿机重新连接，其他矿机保持原线路。
5. 观察面板中的提交、接受和拒绝数量。至少出现 1 个已接受 Share 后，“全量切换”按钮才会启用。
6. 点击“全量切换”。系统只重连这个端口上的矿机，其他端口不受影响。
7. 如果新线路效果不好，点击“恢复上一次”，即可把该端口恢复到切换前的矿池。

面板校验的是矿池地址中已登记的算法资料，Stratum V1 本身通常不会可靠报告算法。因此，新建自定义矿池时必须正确选择算法。即使算法相同，也建议使用真实矿池账号检测，并完成单机试切后再全量切换。

“建议观察 10/20/30 分钟”只用于提醒，不会自动结束试切。这样即使管理页面暂时关闭，测试矿机也不会在无人观察时自行换回并造成线路混杂。

每台 VPS 都有独立配置。启用了备用 VPS 时，需要分别登录主 VPS 和每台备用 VPS 做相同切换。
