...
cd "$(dirname "$0")"
for file in stratum_admin_v3.py stratum_inspector.py endpoint_monitor.py security_monitor.py v3_manager.py; do
  test -f "./$file"
done
...
python3 -m py_compile ./stratum_admin_v3.py ./stratum_inspector.py ./endpoint_monitor.py ./security_monitor.py ./v3_manager.py "$secure_server"
systemctl stop stratum-security-monitor.service
systemctl stop stratum-secure-relay.service
trap 'systemctl start stratum-secure-relay.service >/dev/null 2>&1 || true; systemctl start stratum-security-monitor.service >/dev/null 2>&1 || true' EXIT
install -m 0755 ./stratum_admin_v3.py ./stratum_inspector.py ./endpoint_monitor.py ./security_monitor.py ./v3_manager.py /opt/stratum-admin/
...
PYTHONPATH=/opt/stratum-admin python3 -c "from security_monitor import BASELINE_FILE,atomic_write,load,snapshot; p=['/opt/stratum-admin/stratum_admin_v3.py','/opt/stratum-admin/stratum_inspector.py','/opt/stratum-admin/endpoint_monitor.py','/opt/stratum-admin/security_monitor.py','/opt/stratum-admin/templates/v3_dashboard.html','/opt/stratum-admin/static/v3.css','/opt/stratum-secure-server.py','/etc/stratum-inspector.json','/etc/haproxy/haproxy.cfg']; b=load(BASELINE_FILE,{}); b.update(snapshot(p)); atomic_write(BASELINE_FILE,b)"
rm -f "$candidate_inspector" "$candidate_haproxy"
systemctl start stratum-security-monitor.service
trap - EXIT
systemctl --no-pager --full status haproxy.service stratum-inspector-v3.service stratum-endpoint-monitor.service stratum-security-monitor.service stratum-admin.service stratum-secure-relay.service
