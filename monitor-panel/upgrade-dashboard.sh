...
for file in stratum_admin_v3.py security_monitor.py templates/v3_dashboard.html static/v3.css; do
  test -f "./$file" || { echo "Missing required file: $file" >&2; exit 1; }
done

stamp=$(date +%Y%m%d-%H%M%S)
backup="/root/stratum-dashboard-backup-$stamp"
install -d -m 0700 "$backup"
cp -a /opt/stratum-admin/stratum_admin_v3.py /opt/stratum-admin/security_monitor.py "$backup/"
...
python3 -m py_compile ./stratum_admin_v3.py ./security_monitor.py
...
install -m 0755 ./stratum_admin_v3.py ./security_monitor.py /opt/stratum-admin/
...
PYTHONPATH=/opt/stratum-admin python3 -c "from security_monitor import BASELINE_FILE,atomic_write,load,snapshot; p=['/opt/stratum-admin/stratum_admin_v3.py','/opt/stratum-admin/security_monitor.py','/opt/stratum-admin/templates/v3_dashboard.html','/opt/stratum-admin/static/v3.css']; b=load(BASELINE_FILE,{}); b.update(snapshot(p)); atomic_write(BASELINE_FILE,b)"

systemctl restart stratum-admin.service
systemctl start stratum-security-monitor.service
trap - EXIT
systemctl --no-pager --full status stratum-admin.service stratum-security-monitor.service
echo "Dashboard upgrade complete. Backup: $backup"
