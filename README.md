...
当前 Windows 客户端版本为 `2.3.12`，管理面板为 `3.2.14`，VPS 加密入口为 `2.2.4`。Windows 客户端设置页可在管理员控制的本机直接打开。VPS 通知可按严重级别选择渠道、设置免打扰时段；严重告警始终发送，全部渠道失败时在面板保留醒目提示直到任一渠道恢复。发布状态见[更新计划](docs/improvement-roadmap.md)，各组件版本定义见 [version.json](version.json)。
