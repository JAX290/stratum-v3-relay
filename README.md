...
当前 Windows 客户端版本为 `2.3.12`，管理面板为 `3.2.15`，VPS 加密入口为 `2.2.4`。Windows 客户端设置页可在管理员控制的本机直接打开。VPS 总览可同时查看两台服务器的核心服务、版本、线路一致性和同步新鲜度；通知支持按严重级别选择渠道和设置免打扰。发布状态见[更新计划](docs/improvement-roadmap.md)，各组件版本定义见 [version.json](version.json)。
