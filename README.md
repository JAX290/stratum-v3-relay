...
当前 Windows 客户端版本为 `2.2.1`。各组件版本定义见 [version.json](version.json)。
...
- [管理员操作指南](docs/administrator-guide.md)：新增、改名、删除矿场和日常检查。
...
| 双 VPS 线路同步 | 保持两台 VPS 的转发线路一致，减少切换后的配置差异 | 管理面板 → 设置 → 双 VPS 线路同步 |
...
**删除会撤销原密钥的新连接和重连权限，可能导致矿场中转掉线。请先停用或迁移对应客户端。** 已有连接可能暂时继续，不代表删除后仍可正常使用。新增、改名和删除的步骤见[管理员操作指南](docs/administrator-guide.md)。
...
| 多渠道通知 | 将矿池、线路和安全异常发送到企业微信、钉钉或邮箱；支持页面配置和单独测试 | 管理面板 → 设置 → 通知渠道 |
...
上述表格列出已实现的功能。VPS 全面一键诊断、面板一键安全修复等后续计划见[开发路线](docs/improvement-roadmap.md)。
...
| 文档 | 内容 |
| --- | --- |
| [管理员操作指南](docs/administrator-guide.md) | 日常值守、矿场管理、密钥使用和删除确认 |
...
| [开发路线](docs/improvement-roadmap.md) | 后续开发计划与完成状态 |
