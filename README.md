...
当前 Windows 客户端版本为 `2.3.11`，管理面板为 `3.2.11`，VPS 加密入口为 `2.2.3`。VPS 运维中心提供全面诊断、稳定问题编号、脱敏支持包和固定安全修复；修复前保存最近 10 份备份，失败时恢复，连续失败 3 次会停止后续重试。发布状态见[更新计划](docs/improvement-roadmap.md)，各组件版本定义见 [version.json](version.json)。
