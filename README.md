...
下面是面向不懂程序的值守人员整理的客户端改进方向，均为**后续计划，当前版本尚未全部提供**。实施状态和顺序以[开发路线](docs/improvement-roadmap.md#windows-值守电脑自助维护)为准。

| 优先级 | 计划功能 | 值守人员能得到什么 |
| --- | --- | --- |
| P1 | Windows 服务模式和独立看门狗 | 无需登录 Windows，程序退出、卡死或电脑重启后自动恢复；反复失败时停止重启并给出原因 |
| P1 | 一键检查并修复 | 一个按钮检查网络、端口、防火墙、主备 VPS、证书、密钥、进程和版本，只处理安全且可回退的问题 |
| P1 | 最后可用配置和自动回退 | 新设置无法工作时自动恢复上一份已经验证可用的配置，避免因填错参数导致整场掉线 |
| P1 | 更稳健的主备切换 | 增加连续失败阈值、恢复观察期和切换冷却，避免网络抖动时在两台 VPS 之间来回切换 |
| P1 | 断网、休眠和网卡变化恢复 | Windows 恢复网络、唤醒或 IP 变化后主动重连，并明确显示恢复进度和结果 |
| P2 | 值守模式首页 | 默认只显示“是否正常、影响多少矿机、现在做什么”，高级参数放到单独页面 |
| P2 | 配置导入、备份和迁移向导 | 用加密接入文件部署新电脑；更换电脑时先校验资料和连通性，成功后再切换 |
| P2 | 签名自动升级与一键回退 | 校验发布者和文件签名后升级；启动失败可退回上一版本，并显示通俗结果 |
| P2 | VPS 远程查看客户端状态 | 管理员在 Tailscale 面板看到客户端在线、版本、当前 VPS、最近 Share 和故障摘要 |
| P3 | 值守员与管理员两种模式 | 值守员只使用日常检查和安全修复；管理员才能修改端口、密钥和证书等高风险设置 |
| P3 | 脱敏技术支持包 | 一键导出版本、诊断、事件和必要日志，自动排除共享密钥、密码和证书私钥 |

每项自动修复都必须先说明将执行什么，执行前备份，执行后重新检查；连续失败时停止操作并提示联系技术人员。
