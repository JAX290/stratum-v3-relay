...
当前 Windows 客户端版本为 `2.3.12`，管理面板为 `3.2.13`，VPS 加密入口为 `2.2.4`。Windows 客户端设置页可在管理员控制的本机直接打开。VPS 通知会带通俗原因、影响范围、稳定问题编号和处理入口，恢复时发送闭环，并在面板显示各渠道最近结果。发布状态见[更新计划](docs/improvement-roadmap.md)，各组件版本定义见 [version.json](version.json)。
