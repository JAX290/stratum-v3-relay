...
当前 Windows 客户端版本为 `2.3.12`，管理面板为 `3.2.16`，VPS 加密入口为 `2.2.4`。Windows 客户端设置页可在管理员控制的本机直接打开。VPS 总览可同时查看两台服务器状态，并显示每日巡检结论和最近 7 天地址稳定性；通知支持按严重级别选择渠道和设置免打扰。发布状态见[更新计划](docs/improvement-roadmap.md)，各组件版本定义见 [version.json](version.json)。
