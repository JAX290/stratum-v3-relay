...
```text
secure-relay/client/木林森中转2.1.6正式版.exe
```

当前开发中的 Windows 客户端版本为 `2.2.0`。运行后可直接在窗口标题看到“木林森中转 v2.2.0”；主界面、矿机状态、备用 VPS 设置和诊断报告窗口使用同一个程序图标。正式发布时构建脚本会生成 `木林森中转2.2.0正式版.exe`，测试阶段使用 `木林森中转2.2.0测试版.exe`。
