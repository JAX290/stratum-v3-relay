...
矿机
  │  局域网内普通 Stratum
  ▼
Windows 值守电脑（木林森中转 x.x.x 正式版.exe）
  │  TLS 加密连接
  ▼
VPS（加密入口 + V3 管理面板）
  │  Stratum
...
脚本会先要求设置独立的值守账号和至少 12 个字符的登录密码，再询问“只读面板域名”。第一台 VPS 填写 `status1.mulinsen.win`，第二台填写 `status2.mulinsen.win`。只填写域名，不要带 `https://`、端口或结尾斜杠。脚本会自动：
...
当前 Windows 客户端版本为 `2.2.1`。运行后可直接在窗口标题看到“木林森中转 v2.2.1”。这一版对不可达或静默不响应的备用 VPS 设置了完整超时，限制同时建连数量，并限制日志、Share历史和待确认记录的内存占用。普通软件异常会写入本机 `crash.log`，程序退出3秒后自动启动；10分钟内最多自动恢复3次，避免故障循环。勾选“开机自动启动”后，恢复启动时也会自动开始中转。
...
后续的一键运维功能会按照改进路线中的“P4：非技术管理员的一键运维”实施，目标是让日常管理员直接看到影响范围，并通过诊断、修复、验证和失败回退完成常见故障处理。
