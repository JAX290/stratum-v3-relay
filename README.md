...
当前 Windows 客户端版本为 `2.3.1`。本版新增最后可用配置：完整检查本地端口及所有启用 VPS 的证书、密钥和连通性，验证失败时自动恢复上一份有效配置；2.3.0 的 Windows 服务和看门狗继续保持兼容。发布状态见[更新计划](docs/improvement-roadmap.md)，各组件版本定义见 [version.json](version.json)。
